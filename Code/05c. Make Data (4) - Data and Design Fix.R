...
inds <- read.delim(unz(tmp, "Siccodes48.txt"), header = FALSE)
...
inds <- inds %>% 
  ...
  mutate(name_id = ifelse(sum(str_detect(word(name, 1), "-")) >= 1, 0, 1)) %>% 
  # get the id numbers
  mutate(ff_ind_num = ifelse(name_id == 1, as.numeric(str_sub(name, 1, str_locate(name, " ")[1] - 1)), as.numeric(NA))) %>% 
  # get the short name
  ...
  ungroup() %>%
  fill(ff_ind_num) %>% fill(ind_abbrev) %>% fill(ind_name) %>%
  ...
  mutate(sic_low = ifelse(name_id == 1, as.numeric(NA), as.numeric(str_sub(name, 1, str_locate(name, "-")[1] - 1))),
         sic_high = ifelse(name_id == 1, as.numeric(NA), as.numeric(str_sub(name, str_locate(name, "-")[1] + 1, 
                                                                            str_locate(name, "-")[1] + 4)))) %>%
  ungroup() %>%
  select(ff_ind_num, ind_abbrev, ind_name, sic_low, sic_high) %>% 
  # expand from sic low to sic high by row so we can do a 1:1 merge
  rowwise() %>% 
  do(tibble(ff_ind_num = .$ff_ind_num, ind_abbrev = .$ind_abbrev, 
            ind_name = .$ind_name,
            sic = seq(.$sic_low, .$sic_high))) %>% 
  # make sich a four digit string
  mutate(sich = ifelse(!is.na(sic),
                       paste(rep("0", 4 - nchar(sic)), sic, sep = ""), NA)) %>% 
  ungroup()

# merge in
comp <- comp %>% left_join(inds, by = c("sich"))
...
comp <- comp %>% 
  mutate(industry_year = group_indices(., industry, year),
         industry_ff_year = group_indices(., ff_ind_num, year),
...
# save the data
saveRDS(comp, here::here("Data/COMPILED", "data4.rds"))
